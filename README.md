...
- **Waitlist backend (wired)** — the form POSTs to the `waitlist` edge function on the `simplystroke` Supabase project (source + migration in `supabase/`); signups land in the `public.waitlist` table (service-role-only access, honeypot + dedupe). `NEXT_PUBLIC_WAITLIST_ENDPOINT` overrides the endpoint if ever needed.
